<template>
  <div class="my-follow">
    <Card :title="!showNotes ? '我的关注' : `${currentAuthor.userName}的文章`">
      <template #right>
      ...
      <template v-if="notEmpty(list)">
        <ul class="star-list" v-if="!showNotes">
          <li class="star-item" v-for="i in list" :key="i.id">
            ...
            <button class="btn" @click="handleFocus(i)">
            ...
            >
              <i class="el-icon-user-solid" style="marginright: 2px"></i
              >Ta的信息
            </button>
          </li>
        </ul>
            ...
            <div class="note-brief" v-for="i in list" :key="i.id">
              <div class="brief-box">
                <h3 @click="gotoDetail(i)">
                  {{ i.title }}
                  <span class="fa fa-jpy" v-if="i.type === 1">{{
                    i.price
                  }}</span>
                </h3>
                <p>{{ i.title }}</p>
              ...
              <div class="cover-box">
                <img
                  src="http://www.bianbiangou.cn/index/ICON2.png"
                  alt=""
                  @click="gotoDetail(i)"
                />
              </div>
