    ...
    >
      ...
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="80px" class="post-form">
        ...
        <el-form-item label="文章标签" prop="tag">
          <el-select v-model="ruleForm.tag" placeholder="请选择文章标签" @change="tagChange" @keyup="keyup">
            <el-option v-for="tag in tags" :key="tag.id" :label="tag.name" :value="tag.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="笔记类型" prop="type">
          <el-radio-group v-model="ruleForm.type">
            <el-radio v-for="i in POST_STATE" :key="i.type" :label="i.type">{{ i.text }} </el-radio>
          </el-radio-group>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="sendPost(0)">{{ isEdit ? '修改文章' : '发布文章' }} </el-button>
      </span>
    </el-dialog>
  </div>
</template>
